...
export default function useActivity() {
  ...
  const [activities, setActivities] = useState(() => {
    const stored = localStorage.getItem('azentrix_activities');
    if (stored) {
      try {
    ...
    return [
      { id: '1', text: 'Task board environment initialized', time: new Date(Date.now() - 3600000 * 2).toISOString(), type: 'system' },
      { id: '2', text: 'System audit logs verified', time: new Date(Date.now() - 3600000 * 1.5).toISOString(), type: 'system' }
    ];
  });

  useEffect(() => {
    localStorage.setItem('azentrix_activities', JSON.stringify(activities));
  }, [activities]);
